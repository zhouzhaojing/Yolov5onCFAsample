...
def main( ):
    ...
    #.close()
    f.write("Yolov5_begin /n")
    # Step 1. Initialize OpenVINO Runtime core
    core = ov.Core()
    # Step 2. Read a model
    model = core.read_model(str(Path("./data/yolov5.xml")))
    f.write("Model: {} \n".format(model))
   
    # Step 3. Read input image
    img = cv2.imread(str(Path("./data/000000000312.jpg")))
    # resize image
    ...
    for prediction in detections:
        ...
        if confidence >= CONFIDENCE_THRESHOLD:
            ...
            print( cv2.minMaxLoc(classes_scores))
    ...
    cv2.imwrite("/mount_folder/detection_python.png", img)
